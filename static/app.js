...
function ensureFileInputHasActive(){
  if(!fileInput || !activeFile) return;
  ...
  try{
    ...
    // Ignore if the browser blocks programmatic assignment; we'll send the cached file reference.
  }
}
...
function initDragAndDrop(){
  ...
  dropZone.addEventListener("click", (event) => {
    ...
    if(target === btnBrowse) return;
    if(fileInput && (target === fileInput || fileInput.contains(target))){
      return;
    }
    fileInput?.click();
  });
